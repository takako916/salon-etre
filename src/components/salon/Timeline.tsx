...
const posters: YearPoster[] = [
  ...
  { year: 2026, img: "6.png", affiche: "affiche2026.png" },
];
...
export default function Timeline() {
  ...
  return (
    <section className="mx-auto w-full max-w-7xl px-2 md:px-8 py-16">
      <h2 className="text-2xl md:text-3xl font-bold text-center mb-8 text-salon-accent font-playfair">
      ...
      <div className="relative w-full flex justify-between items-center mb-8">
        ...
        {posters.map((p, i) => (
          <span
            ...
            className={
              i === posters.length - 1
                ? "font-medieval text-2xl md:text-4xl font-bold text-salon-highlight bg-white px-2 relative z-10 text-center"
          ...
          <div key={p.year} className="flex flex-col items-center w-9 md:w-12" style={{ width: i === posters.length - 1 ? 96 : 80 }}>
            {p.affiche ? (
              <Image
                ...
                unoptimized
                className="rounded shadow-md transition-transform duration-300 hover:scale-175 hover:z-10 cursor-pointer"
                style={{ zIndex: 10 }}
